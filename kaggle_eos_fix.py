...
kaggle_eos_fix.py
-----------------
Self-contained EOS-fix resume fine-tuning script for Kaggle T4.
...
# PATHS
# =============================================================================
INPUT_DIR  = Path("/kaggle/input/morpheus-molgen")
...
CKPT_SAVE      = OUTPUT_DIR / "best_finetuned_eos_fix.pt"
LOG_PATH       = OUTPUT_DIR / "finetune_eos_fix_log.csv"
PLOT_DIR       = OUTPUT_DIR / "plots_eos_fix"
SAMPLES_PATH   = OUTPUT_DIR / "finetune_eos_fix_samples.txt"
...
CONFIG = {
    ...
    "batch_size"      : 64,        # T4 16 GB can handle 64
    "learning_rate"   : 1e-5,
    "weight_decay"    : 0.01,
    "max_grad_norm"   : 1.0,
    "num_epochs"      : 3,
...
class ConditionalSELFIESDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        token_ids   = self.tokenizer.encode(selfies_str)[:self.max_length]
...
def save_cfg_samples(model, tokenizer, hf_tokenizer, device):
    ...
    for prompt in unique_prompts:
        ...
        for r in [r for r in results if r["prompt"] == prompt]:
            tag = "✓" if r["valid"] else "✗"
            smiles_or_sel = r["smiles"] if r["valid"] else r["selfies"]
            extra = f"  MW={r['mw']} QED={r['qed']}" if r["valid"] else ""
            print(f"  {tag} {r['token_count']} tok | {smiles_or_sel[:60]}{extra}")
    print(f"\nSaved → {SAMPLES_PATH}")
    model.train()
...
def train():
    ...
    print(f"[ckpt] Resuming from {CKPT_RESUME} ...")
    ckpt = torch.load(CKPT_RESUME, map_location=device)
    missing, unexpected = model.load_state_dict(ckpt["model"], strict=False)
    prev_val = ckpt.get("val_loss", float("inf"))
    print(f"  missing={len(missing)}, unexpected={len(unexpected)}, "
          f"prev_val_loss={prev_val:.4f}")
    ...
    # --- W&B ---
    if USE_WANDB:
        run = wandb.init(project=CONFIG["wandb_project"], name="eos-fix-resume",
    ...
    log_writer.writeheader(); log_file.flush()
    ...
    for epoch in range(CONFIG["num_epochs"]):
        ...
        for batch in progress_bar:
            ...
            if global_step % CONFIG["log_every"] == 0:
                ...
                if USE_WANDB: wandb.log({"train/loss": loss.item(), "train/lr": lr}, step=global_step)
                log_writer.writerow({
            ...
            if global_step % CONFIG["val_every"] == 0:
                ...
                if USE_WANDB: wandb.log({"val/loss": val_loss}, step=global_step)
                log_writer.writerow({
                    "epoch": epoch+1, "step": global_step,
                ...
                if val_loss < best_val_loss:
                    ...
                    if USE_WANDB:
                        artifact = wandb.Artifact(
                            name=f"eos-fix-valloss{val_loss:.4f}", type="model",
        ...
        if (epoch + 1) % 3 == 0:
            periodic_path = OUTPUT_DIR / f"eos_fix_epoch_{epoch+1}.pt"
            torch.save({
                "epoch": epoch+1, "step": global_step,
            ...
            print(f"  Periodic checkpoint → {periodic_path.name}")
